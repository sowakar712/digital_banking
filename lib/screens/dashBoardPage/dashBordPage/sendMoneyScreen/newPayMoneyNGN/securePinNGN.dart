...
class SecurePayNGN extends StatefulWidget {
  String Name;
  String acNo;
  String refMsg;
  String BankCode;
  String AcType;
  SecurePayNGN(this.Name,this.acNo,this.refMsg,this.BankCode,this.AcType);

  @override
  State<SecurePayNGN> createState() => _SecurePayNGNState();
}

class _SecurePayNGNState extends State<SecurePayNGN> {
  String pincode="";
  String text = '';
  List<String> OTPText=['','','','','',''];
  ...
  Future registerNGNbeneficiary() async {
    try {
      var headers={
        'Authorization':"Bearer " + "${Constant.token}",
      };
      var request = http.MultipartRequest('POST', Uri.parse(Constant.addNGBeneficiary));
      request.fields.addAll({
        'account_name': widget.Name,
      ...
      if (response.statusCode == 200) {
        ...
        var res=await Get.to(()=>SuccessAddPayNGN(widget.Name));
        if(res!=null){
